...
def make_template_bank(num_temp):
    TemplateBank = torch.tensor([[0, 0, 0, 0]], device='cpu') 
    while TemplateBank.size()[0] < num_temp:
        rows, columns = TemplateBank.size()
        num_1 = torch.tensor([-1.74, -1.74], device='cpu')
        num_2 = torch.tensor([3.47, 3.47], device='cpu')
        ref_mass = torch.rand(2, device='cpu').multiply(num_2).add(num_1) #(r1 - r2) * torch.rand(a, b) + r2
        num_3 = torch.tensor([2, 2], device='cpu')
        num_4 = torch.tensor([-1, -1], device='cpu')
        ref_spin = torch.rand(2, device='cpu').multiply(num_3).add(num_4)
...
output_sngl_inspiral_table('Ligo1000TemplateBank.xml', my_bank, None, None)