...
while len(TemplateBank) < 10000:
    ref_mass = np.random.uniform(2.0, 100, size=(2))
    ref_spin = np.random.uniform(-0.99, 0.99, size=(2))

    accept = True
    for parameters in TemplateBank:
        template_reference, _ = get_fd_waveform(approximant='IMRPhenomXAS', mass1=ref_mass[0], mass2=ref_mass[1], spin1z=ref_spin[0], spin2z=ref_spin[1], delta_f=delta_f, f_lower=12)
        template, _ = get_fd_waveform(approximant='IMRPhenomXAS', mass1=parameters[0], mass2=parameters[1], spin1z=parameters[2], spin2z=parameters[3], delta_f=delta_f, f_lower=12)
    ...
    if accept == True:
        TemplateBank.append([ref_mass[0], ref_mass[1], ref_spin[0], ref_spin[1]])
